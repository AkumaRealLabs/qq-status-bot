...
func TestSchedulerAndCLIProxyMergeUpdate(t *testing.T) {
	oldS := SchedulerConfig{BaseURL: "https://s", UserID: "1", AccessToken: "tok"}
	gotS := SchedulerConfig{BaseURL: " https://s2/ ", UserID: " 2 ", AccessToken: ""}.MergeUpdate(oldS)
	if gotS.BaseURL != "https://s2" || gotS.UserID != "2" || gotS.AccessToken != "tok" {
