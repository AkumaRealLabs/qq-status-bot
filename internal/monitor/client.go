...
func (c Client) Check(ctx context.Context, u *Upstream, probeModel, selectedKey string) (CheckResult, error) {
	...
	if selectedKey == "" {
		out.Probe = ProbeResult{Success: false, Error: "未选择 Key"}
		return out, nil
	}
...
func (c Client) newapiKeys(ctx context.Context, u *Upstream) ([]APIKey, error) {
	...
	for _, item := range items {
		...
		if id != "" && (key == "" || strings.Contains(key, "*")) {
			...
			err := c.doJSON(ctx, http.MethodPost, joinURL(u.BaseURL, "/api/token/"+url.PathEscape(id)+"/key"), nil, newapiHeaders(u), &full)
			if err != nil {
				return nil, markAuth(err)
			}
...
func (c Client) newapiGroups(ctx context.Context, u *Upstream) map[string]upstreamGroup {
	groups, err := c.fetchNewapiGroups(ctx, u, "/api/user/self/groups")
	if err == nil {
		return groups
	}
	groups, err = c.fetchNewapiGroups(ctx, u, "/api/user/groups")
	if err == nil {
		return groups
	}
	return nil
...
func (c Client) doJSON(ctx context.Context, method, url string, body any, headers map[string]string, out any) error {
	var r io.Reader
	if body != nil {
